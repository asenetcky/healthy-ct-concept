...
#| message: false
#| warning: false
#| fig-cap: "___Source:___ [@ct-rdl-2024]"
...
# what probably needs to happen is take current pct/2 and then add it to
# the full pcts of the previous levels

# plot
blues <- 
  list_color_hex_codes$list_ct_primary_colors |> 
  purrr::map_vec(purrr::pluck) |> 
  unname()
...
ggplot(edu_attn, aes(x = grp, y = pct, fill = degree)) +
  ...
  geom_text(aes(label = label, y = label_y), size = 2.7, col = "white") +
  coord_flip() +
